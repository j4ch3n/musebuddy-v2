...
export type ChordHand = Hand | 'all';
...
export function ChordHandShapeCue({ hand = 'all', notes }: ChordHandShapeCueProps) {
  const hands: readonly Hand[] = hand === 'all' ? ['left', 'right'] : [hand];
  const notesByHand = new Map<Hand, readonly ChordDisplayNote[]>(
    hands.map((handName) => [handName, notes.filter((note) => note.hand === handName)]),
  );
  const cueByHand = new Map(derivePianoHandShapeCues(notes).map((cue) => [cue.hand, cue]));
  const description = hands
    .map((handName) => {
      const handNotes = notesByHand.get(handName) ?? [];
      return `${handName} hand: ${handNotes.length ? handNotes.map(pitchName).join(', ') : 'No keys'}`;
    })
    .join('; ');
  ...
  return (
    <View accessibilityLabel={`Hand shape: ${description}`} accessible style={styles.cue}>
      <View style={styles.hands}>
        {hands.map((handName) => (
          <HandDiagram
            cueName={cueByHand.get(handName)?.name ?? 'No keys'}
            hand={handName}
            key={handName}
          />
        ))}
...
function HandDiagram({
  ...
  return (
    <View accessibilityElementsHidden style={styles.handDiagram}>
      <Svg height={56} viewBox="0 0 100 118" width={48}>
      ...
      <View style={styles.handCopy}>
        <Text style={styles.handCue}>{cueName}</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  handCopy: { flexShrink: 1, gap: 1 },
  handCue: { color: museBuddyColors.pine, fontSize: 11, fontWeight: '800' },
  handDiagram: { alignItems: 'center', flex: 1, flexDirection: 'row', gap: 4, minWidth: 0 },
  hands: { alignItems: 'center', alignSelf: 'stretch', flexDirection: 'row', gap: 4 },
});
