drop function public.get_daily_training_session_candidate();
...
as $$
  ...
  select
    ...
    (
      select jsonb_agg(
        jsonb_build_object(
          'degree', tone.value ->> 'degree',
          'pitch', tone.value ->> 'pitch',
          'pitchClass', tone.value -> 'pitchClass',
          'importance', tone.value ->> 'importance',
          'explanation', tone.value ->> 'explanation'
        )
        order by tone.ordinality
      )
      from jsonb_array_elements(profiles.tones) with ordinality as tone(value, ordinality)
    ) as chord_tones,
    arrangements.song_id,
